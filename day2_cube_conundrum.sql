...
CREATE TEMP TABLE question(red bigint, green bigint, blue bigint);

COPY question FROM STDIN (FORMAT CSV);
12,13,14
\.
...
SELECT
	games[1] as game,
	COALESCE(NULLIF(regexp_replace(colors[1], '(\d+) red', '\1'), colors[1]), NULLIF(regexp_replace(colors[2], '(\d+) red', '\1'), colors[2]), NULLIF(regexp_replace(colors[3], '(\d+) red', '\1'), colors[3]), '0')::bigint as red,
	COALESCE(NULLIF(regexp_replace(colors[1], '(\d+) green', '\1'), colors[1]), NULLIF(regexp_replace(colors[2], '(\d+) green', '\1'), colors[2]), NULLIF(regexp_replace(colors[3], '(\d+) green', '\1'), colors[3]), '0')::bigint as green,
	COALESCE(NULLIF(regexp_replace(colors[1], '(\d+) blue', '\1'), colors[1]), NULLIF(regexp_replace(colors[2], '(\d+) blue', '\1'), colors[2]), NULLIF(regexp_replace(colors[3], '(\d+) blue', '\1'), colors[3]), '0')::bigint as blue
FROM
	input,
	regexp_matches(line, '^Game (\d+): (.*)$') as games,
	regexp_split_to_table(games[2], ';\s?') as rounds,
	regexp_split_to_array(rounds, ',\s?') as colors
),
valid_games AS (	
SELECT
	game
...
JOIN	
	question q
ON
	(r.blue > q.blue OR r.red > q.red OR r.green > q.green)
)
SELECT sum(game::bigint) as result_1 FROM valid_games;
...
per_game AS (
SELECT
	game, max(red) * max(green) * max(blue) as power
FROM
	round
GROUP BY
	game)
SELECT
	sum(power) as result_2
FROM
	per_game; 
