...
CREATE TEMP TABLE calibration(line TEXT);

\COPY calibration FROM STDIN;
...
SELECT
	sum(
	(CASE WHEN COALESCE(array_to_string(matches, '')::bigint,0) < 10
		THEN COALESCE(array_to_string(matches, '')::bigint,0) * 11
		ELSE COALESCE(array_to_string(matches, '')::bigint,0) END))
FROM
	calibration,
	regexp_matches(line, '^.*?(\d).*(\d).*|.*(\d).*') as matches;

-- part2
SELECT
	sum(
	CASE WHEN COALESCE(replace(replace(replace(replace(replace(replace(replace(replace(replace(array_to_string(matches, ''),'one', '1'),'two','2'),'three','3'),'four','4'),'five','5'),'six','6'),'seven','7'),'eight','8'),'nine','9')::bigint,0) < 10
	THEN
		COALESCE(replace(replace(replace(replace(replace(replace(replace(replace(replace(array_to_string(matches, ''),'one', '1'),'two','2'),'three','3'),'four','4'),'five','5'),'six','6'),'seven','7'),'eight','8'),'nine','9')::bigint,0) * 11
		ELSE
		COALESCE(replace(replace(replace(replace(replace(replace(replace(replace(replace(array_to_string(matches, ''),'one', '1'),'two','2'),'three','3'),'four','4'),'five','5'),'six','6'),'seven','7'),'eight','8'),'nine','9')::bigint,0)
	END)
	FROM
		calibration,
		regexp_matches(line, '^.*?(one|two|three|four|five|six|seven|eight|nine|\d).*(one|two|three|four|five|six|seven|eight|nine|\d).*|.*(one|two|three|four|five|six|seven|eight|nine|\d).*') as matches;
